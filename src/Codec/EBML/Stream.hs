...
newStreamReader = StreamReader [] 0 (Get.runGetIncremental getInitialization)
...
feedReader = go []
  where
    ...
    go [] "" ir = case Get.pushEndOfInput ir.decoder of
        ...
        Get.Done "" _ xs -> ([StreamFrame xs (mconcat $ reverse ir.acc)], Right ir)
    ...
    go chunks bs ir =
        case Get.pushChunk ir.decoder bs of
            ...
            Get.Done leftover consumed xs -> do
                let doneChunk = BS.take (fromIntegral consumed - ir.consumed) bs
                    newChunk = StreamFrame xs (mconcat $ reverse (doneChunk : ir.acc))
